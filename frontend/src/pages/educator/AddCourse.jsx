    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form className='flex flex-col gap-4 max-w-md w-full text-gray-500' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={e => setCourseTitle(e.target.value)} value={courseTitle} type="text" placeholder='Enter course title' className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' required />
        </div>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className='flex items-center justify-between mb-2'>
                      <span>
                        {lectureIndex + 1}. {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl} target="_blank" rel="noopener noreferrer" className='text-blue-500'>Link</a> - {lecture.isPreviewFree ? "Free" : "Paid"}
                      </span>
                      <div className='flex items-center gap-2'>
                        <img src={assets.edit_icon} alt='Edit' className='cursor-pointer w-4' onClick={() => handleLecture('edit', chapter.chapterId, lectureIndex)} />
                        <img src={assets.cross_icon} alt='Delete' className='cursor-pointer w-4' onClick={() => handleLecture('remove', chapter.chapterId, lectureIndex)} />
                      </div>
                    </div>
                  ))}
        ...
        <button type="submit" className="mt-4 bg-green-500 text-white py-2 rounded hover:bg-green-600 transition duration-200">Submit Course</button>
      </form>

      {showPopup && (
        <div className='fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50'>
          <div className='bg-white p-4 rounded text-gray-700 relative w-full max-w-80'>
            <h2 className='text-lg font-semibold mb-4'>{isEditing ? 'Edit Lecture' : 'Add Lecture'}</h2>
